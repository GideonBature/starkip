...
const CONFIG = {
  network: 'mainnet',
  nodeUrl: process.env.MAINNET_NODE_URL || 'https://starknet-mainnet.public.blastapi.io/rpc/v0_7',
...
async function validateEnvironment() {
  ...
  if (!CONFIG.deployerPrivateKey) missing.push('MAINNET_DEPLOYER_PRIVATE_KEY');
  if (!CONFIG.deployerAddress) missing.push('MAINNET_DEPLOYER_ADDRESS');
...
async function declareContract(account, sierra, casm) {
  ...
  const feeOptions = { maxFee: 1000000000000000 };
...
async function saveDeploymentInfo(classHash, contractAddress, deploymentTx) {
  const deploymentInfo = {
    ...
    explorerUrl: `https://starkscan.co/contract/${contractAddress}`
  };
...
async function main() {
  ...
  try {
    ...
    console.log(`🔗 Explorer: https://starkscan.co/contract/${deployResult.contract_address}`);
    console.log('═'.repeat(60));
